...
namespace Microsoft.AspNetCore.Identity.EntityFrameworkCore
{
    ...
    public class UserStore<TUser, TRole, TContext, TKey, TUserClaim, TUserRole, TUserLogin, TUserToken, TRoleClaim> :
        ...
        IProtectedUserStore<TUser>, IUserPhoneNumberStoreEx<TUser>
        where TUser : IdentityUser<TKey>
        where TRole : IdentityRole<TKey>
        where TContext : DbContext
        where TKey : IEquatable<TKey>
        ...
        where TRoleClaim : IdentityRoleClaim<TKey>, new()
    {
        ...
        /// <summary>
        /// Gets the user, if any, associated with the specified phoneNumber.
        /// </summary>
        /// <param name="phoneNumber">The phoneNumber to return the user for.</param>
        /// <param name="cancellationToken">The <see cref="CancellationToken"/> used to propagate notifications that the operation should be canceled.</param>
        /// <returns>
        /// The task object containing the results of the asynchronous lookup operation, the user if any associated with the specified normalized email address.
        /// </returns>
        public virtual Task<TUser> FindByPhoneNumberAsync(string phoneNumber, CancellationToken cancellationToken = default(CancellationToken))
        {
            cancellationToken.ThrowIfCancellationRequested();
            ThrowIfDisposed();
